Puppet::Type.newtype(:um_nagios_host) do
  desc <<-EOT
    Creates a nagios host object with augeas
  ...
  newproperty(:host_name) do
    desc 'short name used to identify the host'
  ...
  newproperty(:initial_state) do
    desc <<-EOT
    By default Nagios will assume that all hosts are in UP states when it starts. You can override the initial state for a host by using this directive. Valid options are: o = UP, d = DOWN, and u = UNREACHABLE.
    EOT
  ...
  newproperty(:flap_detection_options) do
    ...
    validate do |value|
      if !value.match?(/^([odu](,[odu])*)?$/) then
        raise ArgumentError, "flap_detection_options must be a comma separated list of 'o', 'd', 'u' (#{value} provided)"
      end
    end
  end
  ...
  newproperty(:notification_options) do
    ...
    validate do |value|
      if !value.match?(/^([durfs](,[durfs])*)?$/) then
        raise ArgumentError, "notification_options must be a comma separated list of 'd', 'u', 'r', 'f', 's' (#{value} provided)"
      end
    end
  end
  ...
  newproperty(:stalking_options) do
    ...
    validate do |value|
      if !value.match?(/^([odu](,[odu])*)?$/) then
        raise ArgumentError, "stalking_options must be a comma separated list of 'o', 'd', 'u' (#{value} provided)"
      end
    end
  end
  ...
  newproperty(:coords_2d) do
    ...
    This is the 2d_coords nagios option.
    EOT

    validate do |value|
      if !value.is_a?(String) or !value.match(/^[0-9]+,[0-9]+$/) then
        raise ArgumentError, "coords_2d must be a pair in format <int>,<int> (#{value} provided)"
      end
    end
  end

  newproperty(:coords_3d) do
    desc <<-EOT
    ...
    This is the 3d_coords nagios option.
    EOT

    validate do |value|
      if !value.is_a?(String) or !value.match(/^[0-9]+,[0-9]+$/) then
        raise ArgumentError, "coords_3d must be a pair in format <int>,<int> (#{value} provided)"
      end
    end
  end
